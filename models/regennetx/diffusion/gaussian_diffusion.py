...
from models.regennet2.diffusion.nn import mean_flat, sum_flat
from models.regennet2.diffusion.losses import normal_kl, discretized_gaussian_log_likelihood
from models.regennet2.data_loaders.humanml.scripts import motion_process
import models.regennet2.utils.rotation_conversions as geometry
...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        if dataset in ['interhuman']:
            return self.training_losses_xyz(model, x_start, t, model_kwargs, noise, dataset)
        elif dataset in ['interx']:
            return self.training_losses_smplx(model, x_start, t, model_kwargs, noise, dataset)
        else:
            return self.training_losses_smplx(model, x_start, t, model_kwargs, noise, dataset)


    def training_losses_xyz(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        """
        Compute training losses for a single timestep for humanml3d dataset.
        ...
        """
        length = model_kwargs.get('motion_lens', None)
        max_len = x_start.shape[1]
        range_tensor = torch.arange(max_len).unsqueeze(0).to(length.device)  # shape: (1, max_len)
        mask = (range_tensor < length.unsqueeze(1)).unsqueeze(1).unsqueeze(1)  # shape: (bs, 1, 1, max_len)
            
            
        get_xyz = lambda sample: sample[:, :66,:]
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            terms["rot_mse"] = F.mse_loss(target, model_output) # mean_flat(rot_mse)
            ...
            cmotion_xyz = get_xyz(model_kwargs['motion1'])
            ...
            if self.lambda_body > 0.:
                ...
                body_diff = F.mse_loss(target_xyz, model_output_xyz)
    ...
    def training_losses_smplx(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            terms["rot_mse"] = self.masked_l2(target, model_output, mask).mean() # mean_flat(rot_mse)
            ...
            if self.lambda_vel_rcxyz > 0.: # == 0
                if self.data_rep == 'rot6d' and dataset in ['humanact12', 'uestc', 'ntu', 'chi3d']:
                ...
                with torch.autograd.set_detect_anomaly(True):
                    if self.data_rep == 'rot6d' and dataset in ['humanact12', 'uestc', 'ntu', 'chi3d']:
                        ...
                        terms["fc"] = self.masked_l2(pred_vel,
                                                    ...
                                                    mask[:, :, :, 1:])
                    elif self.data_rep == 'xyz' and dataset in ['humanact12', 'uestc', 'ntu', 'chi3d']:
