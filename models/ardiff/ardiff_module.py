...
from models.ardiff.adaln import AdaLNTransformer, SkipAdaLNTransformer
from models.ardiff.mlp import AdaLnMLP
from models.ardiff.textenc import TextEncoder
from models.ardiff.loss import DiffusionLoss
from models.ardiff.ardiff_sampler import ardiff_sampler
from models.ardiff.ardiff import ARDiff
...
class ARDiffModule(LightningModule):
    def __init__(self, cfg):
        ...
        self.loss_fn = DiffusionLoss(**cfg.mf.siloss)
        self.ardiff = ARDiff(cfg.mf)
    ...
    def forward(self, x, timesteps, y):
        ...
        Args:
            ...
            timesteps (Tensor): [B * any, 1] OR [1] , is the timesteps
        ...
        u = self.ardiff.predict_mean_velocity(x=x_t, context=context, timesteps=timesteps)
    ...
    def forward_test_with_init(self, batch):
        ...
        for i in range(1, latents.shape[1]):
            ...
            context = self.ardiff.encode_context(x0=motion_prev, 
                                                  cond_B1D=text_emb[:, :1, :], 
                                                  cmotion=cmotion_emb[:, :i, :])
            y = torch.cat([cmotion_emb[:, i:i+1, :], context[:, -1:, :]], dim=-1)
                
            x_i = ardiff_sampler(model=self, 
                                  latents=latents[:, -1:, :], 
                                  y={'y': y},
                                  cfg_scale=self.cfg_scale, 
                                  num_steps=self.num_steps)
    ...
    def training_step(self, batch):
        ...
        context = self.ardiff.encode_context(x0=mixed_motion_emb[:, :-1, :], 
                                              cond_B1D=text_emb[:, 0, :], 
        ...
        loss_mean_ref = self.loss_fn(model=self, 
                                           x0=motion_emb.reshape(motion_emb.shape[0] * motion_emb.shape[1], 1, -1), 
