...
import math
from models.armflow.adaln import SkipAdaLNTransformer
...
class ARMFlow(nn.Module):
    def __init__(self, cfg):
        ...
        # timestep embedding
        self.r_embedder = TimestepEmbedder(self.d_model * 2)
    ...
    def forward(self, x, cond_B1D, cmotion, r_timesteps, t_timesteps, mask=None):
        """
        Args:
    ...
    def predict_mean_velocity(self, x:torch.Tensor, context:Tensor, r_timesteps:Tensor, t_timesteps:Tensor):
        """
        Args:
        ...
        """
        B = x.shape[0]
        r_emb = self.r_embedder.forward(r_timesteps) # B, d_model
        t_emb = self.t_embedder.forward(t_timesteps) # B, d_model
        if r_timesteps.shape[0] != B:
            r_emb = r_emb.expand(B, -1)
        if t_timesteps.shape[0] != B:
            t_emb = t_emb.expand(B, -1)
        context = context + r_emb.unsqueeze(1) + t_emb.unsqueeze(1) # B, 1, d_model
