...
from models.armflow.adaln import AdaLNTransformer, SkipAdaLNTransformer
from models.armflow.mlp import AdaLnMLP
from models.armflow.textenc import TextEncoder
from models.armflow.loss import SILoss
from models.armflow.armflow_sampler import armflow_sampler
from models.armflow.armflow import ARMFlow
...
class ARMFlowModule(LightningModule):
    ...
    def forward_train(self, x, x_prev, c_prev, y, r, t):
        """
        Args:
            x_t (Tensor): [B, L, d_model] is the noised latent represnetation with no [s]
            x_prev (Tensor): [B, L-1, d_model] is the clean previous latent represnetation with no [s], can't be NULL
            c_prev (Tensor): [B, L-1, d_model], is the previous cmotion, this can't be NULL cond. 
            y (Tensor): [B, 1 + (L-1), d_cond], is the text condition (cat) cmotion to current, can be NULL
            r (Tensor): [B, 1] OR [1] , is the r time step
            t (Tensor): [B, 1] OR [1] , is the t time step
            
        Returns:
            x (Tensor): [B, L, d_model] is the predicted mean velocity
        """
        cond_B1D = y[:, 0, :]
        c_cur = y[:, 1:, :self.d_model]

        mask = self.process_mask()            
        context = self.armflow.encode_context(x0=x_prev, cond_B1D=cond_B1D, cmotion=c_prev, mask=mask)
        x_t = torch.cat([c_cur, x_t], dim=-1) # b, any, 2 * d_model
