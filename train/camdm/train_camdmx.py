...
from models.camdm.camdmx import CAMDM
...
def main():
    ...
    cfg = OmegaConf.load('cfg/camdm/camdmx.yaml')
    ...
    train_dataloader = DataLoader(
        train_dataset,
        batch_size=cfg.camdm.train.batch_size,
        shuffle=True,
    ...
    exp_name = cfg.camdm.name
    model = CAMDM(cfg=cfg)
        
    if os.path.exists('ckpt/' + exp_name + '/last.ckpt'):
        model = CAMDM.load_from_checkpoint(
    ...
    trainer = Trainer(
        ...
        max_epochs=cfg.camdm.train.epochs,
        callbacks=[
            ModelCheckpoint(
                save_top_k=3,
                monitor="eval/FID",
        ...
        check_val_every_n_epoch=5 if not hasattr(cfg.camdm.train, 'check_val_every_n_epoch') else cfg.camdm.train.check_val_every_n_epoch,
        accelerator='cuda',
        devices=1
    )
