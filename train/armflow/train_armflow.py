...
from models.armflow.armflow_module import ARMFlowModule
...
def main():
    ...
    cfg = OmegaConf.load('cfg/armflow/armflow.yaml')
    ...
    model = ARMFlowModule(cfg=cfg)
        
    if os.path.exists('ckpt/' + exp_name + '/last.ckpt'):
        model = ARMFlowModule.load_from_checkpoint(
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ...
            SkipValidationUntil(cfg.mf.train.get('skip_validation_until', 0)),
            MultiMetricCheckpoint(
                dirpath="ckpt/" + exp_name,
                fid_threshold=2.1,
                mmdist_threshold=3.780)
        ],
